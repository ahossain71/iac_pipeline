  ...
  stages {
    stage('Submit Stack') { 
      steps {
        ...
        sh "aws cloudformation deploy --stack-name sundayWebStack --template-file  $workspace/cloudformation/sunday_web_stack.template --region 'us-east-1' --capabilities CAPABILITY_IAM --parameters ParameterKey=InstanceType,ParameterValue=t2.micro ParameterKey=KeyName,ParameterValue='s3://cf-templates-a1jqt245fpux-us-east-1/DevOpsKeyPair.pem' ParameterKey=SSHLocation,ParameterValue=0.0.0.0/0"
      }
    }
  }
}